...
const getStoredSessionId = async () => {
  const existing = await AsyncStorage.getItem('jeddspace_ai_session_id');
  if (existing) return existing;
  const created = `session_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;
  await AsyncStorage.setItem('jeddspace_ai_session_id', created);
  return created;
};

const getStoredSessions = async () => {
  try {
    const sessions = JSON.parse((await AsyncStorage.getItem('jeddspace_ai_sessions')) || '[]');
    return Array.isArray(sessions) && sessions.length ? sessions : [await getStoredSessionId()];
  } catch {
    return [await getStoredSessionId()];
  }
};
...
export interface AnalyticsData {
  topics: Array<{ name: string; percentage: number; count: number }>;
...
export const getRecommendations = async ({ startDate, endDate }: { startDate?: string; endDate?: string }): Promise<Recommendation[]> => {
  ...
  try {
    ...
    const leaveByEmployee: Record<string, Array<{ start_date: string; end_date: string }>> = {};
    ...
    const jobsByEmployee: Record<string, Array<{ start_date: string; end_date: string; status: string }>> = {};
    allJobs?.forEach((job) => {
